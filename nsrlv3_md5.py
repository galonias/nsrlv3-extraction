...
parser = argparse.ArgumentParser(description='Extract unique MD5 combinations from an SQLite database inside a zip file.')
...
with zipfile.ZipFile(args.zip_file) as zip_file:
    ...
    output_file_name = os.path.splitext(os.path.basename(db_file_name))[0] + "_md5.txt"
    ...
    with zip_file.open(db_file_name) as db_file:
        ...
        # Retrieve unique combinations of md5 values
        query = "SELECT DISTINCT md5 FROM DISTINCT_HASH"
